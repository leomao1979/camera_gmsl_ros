...
public :
    ...
    void setColor(const float32_t *color);
    ...
    void renderText(uint32_t textX, uint32_t textY, const float32_t color[4], std::string text,
    ...
    void renderRectangle(const dwRect &rectangle, const float32_t color[4]);
    ...
    void renderLineSegments(const std::vector<dwLineSegment2Df> &segments, float32_t lineWidth, const float32_t *color);

    void renderPolyline(const std::vector<dwVector2f> &points, float32_t lineWidth, const float32_t *color);

    void renderPoints(const std::vector<dwVector2f> &points, float32_t pointSize, const float32_t *color);
