...
void SimpleRenderer::setColor(const float32_t* color)
{
...
void SimpleRenderer::renderText(uint32_t textX, uint32_t textY, const float32_t color[4], std::string text,
dwRendererFonts font)
{
    // store color
    float32_t oldColor[4];
...
void SimpleRenderer::renderRectangle(const dwRect &rectangle, const float32_t color[4])
{
    // store color
    float32_t oldColor[4];
    CHECK_DW_ERROR(dwRenderer_getColor(oldColor, m_renderer));
...
void SimpleRenderer::renderLineSegments(const std::vector<dwLineSegment2Df> &segments,
                                                       float32_t lineWidth, const float32_t *color)
{
    // store
    float32_t oldColor[4];
    CHECK_DW_ERROR(dwRenderer_getColor(oldColor, m_renderer));
