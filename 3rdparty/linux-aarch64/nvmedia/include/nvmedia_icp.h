...
typedef struct {
     ...
     *  Can be calculated using: HTS * VTS * frame rate. */
 ...
 * This function disables CSI to reject the packet from CSI FIFO so CSI does not report
 * false alarms while finishing up the user application.
 * NvMediaICPStop returns NVMEDIA_STATUS_ERROR when there are images in the image capture pool.
 * In this case, to stop the capture, call \ref NvMediaICPGetFrameEx to
 * get the last image.
 ...
 *         Please remove the frames from pool using NvMediaICPReleaseFrame.
 */
NvMediaStatus
 ...
 * \param[in] icp The image capture object to be used.
 * \param[in] millisecondTimeout Time-out in milliseconds
 * \param[in,out] imageGrp The NvMedia image group is ready for use.
 * \return \ref NvMediaStatus The completion status of the operation.
 * \n \ref NVMEDIA_STATUS_OK : image has valid value, it is good to use.
 * \n \ref NVMEDIA_STATUS_NONE_PENDING : image is NULL, there is no fed frame,
 *         internel pool is empty.
...
/**
 * \brief Gets a frame from the internal pool that user fed with NvMediaICPFeedFrame
 * after HW gives up to capture.
 * This function should be called until getting NVMEDIA_STATUS_ERROR.
 * \param[in] icp The image capture object to be used.
 * \param[in] image The image was fed from user but HW gives up to capture.
 * \return \ref NvMediaStatus The completion status of the operation.
...
/**
 * \brief Gets an image group from the internal pool that user fed with
 * \ref NvMediaICPFeedImageGroup after HW gives up to capture.
 * This function should be called until getting NVMEDIA_STATUS_ERROR.
 * \param[in] icp The image capture object to be used.
 * \param[in] imageGrp The image group was fed from user but HW gives up to capture.
 * \return \ref NvMediaStatus The completion status of the operation.
 * \n \ref NVMEDIA_STATUS_OK : returning the frame that user fed.
 * \n \ref NVMEDIA_STATUS_ERROR : nothing to return.
 */
 ...
 * & error status. The client can use the information to take a suitable action upon
 * a CSI capture error.
 * \param[in] icp  A pointer to the image capture object to be used.
